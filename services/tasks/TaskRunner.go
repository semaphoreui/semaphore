...
import (
	...
	"github.com/Digital-Data-Co/forge/services/tasks/hooks"
...
func (t *TaskRunner) run() {
	...
	// Detect and save any files generated by the task
	t.detectAndSaveTaskFiles()
...
// detectAndSaveTaskFiles scans the project directory for files generated during task execution
// and saves them as task artifacts
func (t *TaskRunner) detectAndSaveTaskFiles() {
	if t.Template.App != db.AppAnsible {
		// Only process Ansible tasks for now
		return
	}

	projectDir := util.Config.GetProjectTmpDir(t.Task.ProjectID)
	if projectDir == "" {
		t.Log("Warning: Could not determine project directory for file detection")
		return
	}

	// Common file patterns that Ansible tasks might generate
	filePatterns := []string{
		"*.log",
		"*.json",
		"*.yaml",
	...
	for _, pattern := range filePatterns {
		...
		if err != nil {
			t.Logf("Warning: Error scanning for pattern %s: %v", pattern, err)
			continue
		}
		...
		for _, filePath := range matches {
			...
			// Skip very small files (likely not meaningful artifacts)
			if fileInfo.Size() < 10 {
				continue
			}
			...
			// Save the file as a task artifact
			taskFile, err := fileStorage.StoreFile(t.Task.ProjectID, t.Task.ID, filePath, fileInfo.Name())
			if err != nil {
				t.Logf("Warning: Could not save task file %s: %v", filePath, err)
				continue
			}

			// Save to database
			_, err = t.pool.store.CreateTaskFile(*taskFile)
			if err != nil {
				t.Logf("Warning: Could not save task file to database %s: %v", filePath, err)
				continue
			}

			filesFound++
			t.Logf("Saved task artifact: %s (%d bytes)", fileInfo.Name(), fileInfo.Size())
		}
	}

	if filesFound > 0 {
		t.Logf("Detected and saved %d task artifacts", filesFound)
	}
}
