...
// WebHostURL is the public route to the forge server
...
func loadConfigFile(configPath string) (usedConfigPath *string) {
	...
	if configPath == "" {
		...
		paths := []string{
			...
			"/usr/local/etc/forge/config.json",
			"/etc/forge/config.json",
		}
		for _, p := range paths {
			_, err = os.Stat(p)
			if err != nil {
...
func exitOnConfigFileError(err error) {
	if err != nil {
		exitOnConfigError("Cannot Find configuration! Use --config parameter to point to a JSON file generated by `forge setup`.")
	}
}
...
// CheckUpdate uses the GitHub client to check for new tags in the forge repo
func CheckUpdate() (updateAvailable *github.RepositoryRelease, err error) {
	// fetch releases
	gh := github.NewClient(nil)
	releases, _, err := gh.Repositories.ListReleases(context.TODO(), "forgeui", "forge", nil)
	if err != nil {
		return
	}
