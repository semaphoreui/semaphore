...
func TestValidateTelegramThreadID(t *testing.T) {
	tests := []struct {
		name      string
		threadID  string
		wantError bool
	}{
		{name: "empty is optional", threadID: ""},
		{name: "whitespace is optional", threadID: "  "},
		{name: "positive integer", threadID: "42"},
		{name: "zero", threadID: "0", wantError: true},
		{name: "negative integer", threadID: "-1", wantError: true},
		{name: "decimal", threadID: "1.5", wantError: true},
		{name: "text", threadID: "general", wantError: true},
		{name: "integer overflow", threadID: "9223372036854775808", wantError: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateTelegramThreadID(tt.threadID)
			if tt.wantError {
				assert.Error(t, err)
			} else {
