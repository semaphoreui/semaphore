...
func (env *Integration) Validate() error {
	if env.Name == "" {
		return &common_errors.ValidationError{"No Name set for integration"}
	}
	return nil
}
...
func (env *IntegrationMatcher) Validate() error {
	if env.MatchType == "" {
		return &common_errors.ValidationError{"No Match Type set"}
	} else {
		if env.Key == "" {
			return &common_errors.ValidationError{"No key set"}
		}
		if env.Value == "" {
			return &common_errors.ValidationError{"No value set"}
		}
...
func (env *IntegrationExtractValue) Validate() error {
	if env.ValueSource == "" {
		return &common_errors.ValidationError{"No Value Source defined"}
	}
	...
	if env.ValueSource == IntegrationExtractBodyValue {
		if env.BodyDataType == "" {
			return &common_errors.ValidationError{"Value Source but no body data type set"}
		}

		if env.BodyDataType == IntegrationBodyDataJSON {
			if env.Key == "" {
				return &common_errors.ValidationError{"No Key set for JSON Body Data extraction."}
			}
		}
	}
	...
	if env.ValueSource == IntegrationExtractHeaderValue {
		if env.Key == "" {
			return &common_errors.ValidationError{"Value Source set but no Key set"}
		}
	}
