...
| [Runner executors](runner-executors.md) | `local`, `docker`, and `k8s` executor strategies, runner config, per-template `executor_image`, OSS vs Pro |
...
| [External identity linking](external-identity-linking.md) | LDAP/OIDC identity rows, email-matching modes, identity APIs |
| [Git SSH security](git-ssh-security.md) | SSH host-key verification for git clone/pull via `GIT_SSH_COMMAND` |
| [HA cluster dashboard](ha-cluster-dashboard.md) | Admin cluster routes, task-state snapshot/clear, runner name in task payloads |
| [Input validation](input-validation.md) | Playbook path checks, git URL/branch/commit validation, integration and vault IDOR gates, backup secret exclusion |
| [Prometheus metrics](prometheus-metrics.md) | `/api/metrics` scrape endpoint, Basic Auth, task running/outcome series |
| [Schedules](schedules.md) | Cron and `run_at` schedules, server-side cron validation, timezone |
