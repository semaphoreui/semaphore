...
## Per-template image override

Templates can set `executor_image` (column `project__template.executor_image`,
migration `v2.19.12.sql`) to override the container image for that template's
tasks. The runner receives the full template in `JobData` on poll; Docker and
Kubernetes executors use `template.executor_image` when set, otherwise they fall
back to `runner.executor.docker.image` or `runner.executor.k8s.image`.

| Condition | Image used |
| --- | --- |
| `executor_image` set (non-empty after trim) | Template value |
| `executor_image` empty / omitted | Runner config default |

Constraints:

- **Docker and Kubernetes only** — the local executor ignores `executor_image`.
- **Pro feature** — the Web UI field is enabled when `GET /api/info` reports
  `docker_executor` or `k8s_executor` (`TemplateForm.vue`).
- **Clearing the field** — an empty string in the API or UI is normalized to
  `NULL` (`Template.NormalizedExecutorImage`) so "no override" has one
  representation in the database.
- **Helper image unchanged** — only the main build container image is overridden;
  `helper_image` still comes from runner config (K8s git-clone init).

Example template API fragment:

```json
{
  "name": "Ansible with custom toolchain",
  "app": "ansible",
  "playbook": "site.yml",
  "executor_image": "my-registry/job:ansible-2.16"
}
```

See [Docker images](docker-images.md) for what to put in a custom `job` image.
...
- Template override: `db/Template.go` (`ExecutorImage`, `NormalizedExecutorImage`), `db/sql/migrations/v2.19.12.sql`
...
- Runner poll payload: `api/runners/runners.go` (`prepareRemoteJob` → `runners.JobData.Template`)
