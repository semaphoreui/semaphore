...
| `kind` | Behaviour |
| --- | --- |
| `task` | Runs the linked task template (with optional per-node `task_params` overrides) |
...
Task nodes can pin launch-time values that would normally be chosen when starting
a one-off task. Stored as `task_params_id` → `project__task_params` (migration
`v2.19.11.sql`); exposed on `WorkflowNode` as `task_params` (`db/Workflow.go`).

| Field | Use |
| --- | --- |
| `environment` | JSON survey answers and extra vars for this node |
| `git_branch` | Branch override when the template allows it |
| `inventory_id` | Inventory override when the template allows it |
| `message` | Task message shown in the UI |
| `arguments` | CLI arguments (shell/Terraform) |
| `params` | App-specific overrides (Ansible limit/tags, Terraform flags, …) |

The workflow editor sidebar (`WorkflowEditor.vue`) embeds `TaskParamsForm.vue` —
the same component used for schedules and manual launches. Empty `task_params` on
a new task node defaults to `{}`; approval and note nodes set `task_params` to
`null`.

At run time, `TaskParams.CreateTask(templateID)` builds the `db.Task` record from
the node's stored params. See [survey-and-variable-types.md](survey-and-variable-types.md)
for survey `int` / `enum` types and validation.
