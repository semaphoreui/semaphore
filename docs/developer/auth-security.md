...
LDAP and OIDC logins are resolved through `user__external_identity` rows rather
than email alone. Session and CSRF rules above still apply to linked external
users. See [External identity linking](external-identity-linking.md) for
`external_auth_email_matching` modes, identity APIs, and takeover protections.
...
- External identity resolver: `api/login_identity.go`
