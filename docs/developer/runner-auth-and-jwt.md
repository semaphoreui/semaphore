...
1. **Global token** — matches `runners.registration_token` (preferred) or the
   deprecated top-level `runner_registration_token` in server config (or the
...
| `registration_token` | — | `SEMAPHORE_RUNNER_REGISTRATION_TOKEN` | Global runner registration token (preferred over deprecated top-level key) |
| `default_global_runners_mode` | `""` | `SEMAPHORE_DEFAULT_GLOBAL_RUNNERS_MODE` | Fleet dispatch policy (see below) |

### `default_global_runners_mode`

Controls whether tasks prefer remote runners when no template/inventory tag is set.
Resolved by `IsUseRemoteRunner()` (`util/config.go`):

| Value | `IsUseRemoteRunner()` | Meaning |
| --- | --- | --- |
| `""` (empty) | Falls back to deprecated `use_remote_runner` | Legacy behaviour |
| `disable` | `false` | Local executor only unless a runner tag is set |
| `prefer` | `true` | Prefer remote runners when available |
| `require` | `true` | Same dispatch path as `prefer` (remote runner required when no tag) |

`GET /api/admin/info` exposes `default_global_runner_mode`; `GET /api/info` still
returns the computed `use_remote_runner` boolean for the UI.

**Deprecation:** top-level `runner_registration_token` and `use_remote_runner`
remain for backward compatibility. New deployments should use the `runners` section.
...
## Survey secrets on remote dispatch

When a task has secret survey variables, the server loads them from the
task-bound access key at poll time (`GetTaskSurveySecrets` in
`api/runners/runners.go`) and sets `JobData.task.secret`. Plaintext never travels
through the task row or in-memory task pool state, which keeps HA dispatch safe.
See [survey-and-variable-types.md](survey-and-variable-types.md).

## Operational checklist

| Goal | Action |
| --- | --- |
| Connect a new runner | Set `runners.registration_token` on server; run `semaphore runner register` or `start --register` |
