export default {
  ...
  keyFormString: 'Секретный токен',
  keyFormStringValue: 'Значение секрета',
  keyFormStringHint: 'Один секрет, например токен бота или приложения, хранится в зашифрованном виде.',
  keyFormNone: 'Нет',
  ...
  telegramThreadId: 'Telegram Thread ID (необязательно)',
  telegramThreadIdHint: 'ID темы форума, если в чате включены темы.',
  alerts: 'Оповещения',
  ...
  testAlert: 'Отправить тестовое сообщение',
  testAlerts: 'Проверить все',
  alertCopyName: '{name} копия',
  alertTestSent: 'Тестовое сообщение отправлено в {name}.',
  alertTestAllSent: 'Тестовые сообщения отправлены.',
  alertTestAllMissing: 'Нечего отправлять: включите серверные каналы или создайте включённое оповещение.',
  serverAlertChannels: 'Серверные каналы',
  serverAlertChannelsHint: 'Каналы из конфигурации сервера (config.json) общие для всех проектов. Включите их, чтобы получать через них все результаты задач этого проекта.',
  serverAlertChannelsNone: 'На сервере не настроено ни одного канала.',
  serverAlertChannelsEnable: 'Отправлять оповещения этого проекта в серверные каналы',
  serverAlertTelegramChatHint: 'Чат, в который приходят сообщения этого проекта. Переопределяет общий Telegram-чат и обязателен, если на сервере чат не задан.',
  serverAlertTelegramChatMissing: 'chat ID не задан',
  serverAlertChannelsSaved: 'Настройки серверных каналов сохранены.',
  projectAlerts: 'Оповещения проекта',
  projectAlertsHint: 'Именованные назначения этого проекта. Отметьте оповещение как «по умолчанию», чтобы его использовали все шаблоны, или выбирайте оповещения для каждого шаблона и расписания.',
  noAlertsYet: 'Оповещений пока нет.',
  alertSendOn: 'Отправлять при',
  alertEventSuccess: 'Успех',
  alertEventError: 'Ошибка',
  alertEventWaitingConfirmation: 'Ожидание подтверждения',
  alertEventsHint: 'Шаблон по-прежнему может подавлять уведомления об успехе или ошибке.',
  alertIsDefault: 'По умолчанию в проекте',
  alertIsDefaultHint: 'Шаблоны с настройками проекта по умолчанию отправляют это оповещение.',
  alertWebhookUrl: 'URL вебхука',
  alertUrlOptionalHint: 'Оставьте пустым, чтобы использовать настройки сервера.',
  alertSecretSource: '{label}',
  alertSecretServer: 'Использовать настройки сервера',
  alertSecretOwn: 'Использовать свои',
  alertSecretKeyHint: 'Ключ доступа типа «{type}» из хранилища ключей.',
  alertSecretNoKeys: 'В проекте нет ключей доступа типа «{type}». Создайте его в хранилище ключей.',
  alertSmtpHost: 'SMTP-сервер',
  alertSmtpPort: 'SMTP-порт',
  alertSmtpSender: 'Адрес отправителя',
  alertSmtpSecure: 'Защищённое соединение (SMTPS)',
  alertSmtpTls: 'STARTTLS',
  alertRecipients: 'Получатели',
  alertRecipientsHint: 'Оставьте пустым, чтобы уведомлять участников проекта, включивших оповещения в профиле.',
  alertAddRecipient: 'Добавить получателя',
  alertMessageBody: 'Шаблон сообщения',
  alertMessageBodyHint: 'Go-шаблон. Доступные поля: .Name, .Author, .Project.Name, .Playbook, .ScheduleName, .Task.ID, .Task.URL, .Task.Result, .Task.Desc, .Task.Version, .Task.Duration, .Task.Trigger.',
  alertCustomized: 'изменён',
  alertResetDefaultBody: 'Сбросить к умолчанию',
  templateAlertsDefault: 'Использовать настройки проекта по умолчанию',
  templateAlertsCustom: 'Использовать свой набор оповещений',
  templateAlertsDefaultHint: 'Серверные каналы (если включены для проекта) и оповещения, отмеченные как «по умолчанию».',
  templateAlertsCustomHint: 'Только выбранные оповещения. Если ничего не выбрано, шаблон не отправляет оповещений.',
  scheduleAlertsInherit: 'Использовать оповещения шаблона',
  scheduleAlertsCustom: 'Использовать другой набор оповещений',
  scheduleAlertsHint: 'Только выбранные оповещения. Если ничего не выбрано, расписание не отправляет оповещений.',
  ...
  suppressErrorAlerts: 'Подавить оповещения об ошибках',
