...
- **Syslog is not filtered.** Syslog output is produced by logrus *hooks*
  (`cli/cmd/syslog.go`: the standard `lSyslog` hook and the custom `rfc5424Hook`), which
  format independently of the standard logger's formatter. The `FilteringFormatter` only
  affects the main (stdout) output path. If syslog filtering is desired, the hooks' `Fire`
  must consult the same `Filter` and skip suppressed debug entries. Deferred — call out
  during stage 2 if needed.
