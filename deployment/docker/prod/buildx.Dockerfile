...
RUN apk add --no-cache sshpass git curl ansible mysql-client openssh-client-default tini python3 py3-pip && \
...
RUN pip install aiohttp jmespath matrix-client netaddr passlib pywinrm requests
