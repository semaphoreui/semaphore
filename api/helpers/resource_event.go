...
func RecordProjectMemberEvent(r *http.Request, event audit.ProjectMemberEvent) {
	service, ok := GetFromContext(r, "audit_service").(*audit.Service)
	if !ok || service == nil {
		log.Error("Failed to record project member event: missing audit service")
		return
	}
	requestContext, ok := AuditRequestContextFrom(r)
	if !ok {
		log.Error("Failed to record project member event: missing audit request context")
		return
	}
	user, ok := GetFromContext(r, "user").(*db.User)
	if !ok || user == nil {
		log.Error("Failed to record project member event: missing authenticated user")
		return
	}
	...
	if err := service.RecordProjectMember(event); err != nil {
		log.WithError(err).Error("Failed to record project member event")
	}
}
