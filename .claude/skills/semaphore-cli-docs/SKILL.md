...
description: >
  Generate or update the CLI documentation under `docs/docs/reference/cli/` from the Cobra command code in
...
`docs/docs/reference/cli/` and the CLI index `docs/docs/reference/cli/README.md`. Every page is **derived from the
...
> **`docs/docs/reference/cli/commands.md` is not yours.** It is generated by `tools/clidocs`, which walks the real
> Cobra tree and emits every command and flag with its Cobra help string. Run `task docs:gen` for it and never edit
> it by hand. This skill owns the *task-oriented* pages next to it: what each command group is for, what the
> arguments mean in practice, what the command actually does to the database, and worked examples. Do not try to
> reproduce an exhaustive flag inventory here — link to `/reference/cli/commands` instead and spend the words on
> behaviour the help strings cannot express.
...
|---------------------------|----------|
| `root.go` | `docs/docs/reference/cli/README.md` — global flags, command overview |
| `server.go`, `setup.go`, `version.go` | `docs/docs/reference/cli/README.md` — operational commands |
| `user.go`, `user_add.go`, `user_change.go`, `user_delete.go`, `user_get.go`, `user_list.go`, `user_token.go`, `user_totp.go` | `docs/docs/reference/cli/users.md` |
| `project.go`, `project_export.go`, `project_import.go` | `docs/docs/reference/cli/projects.md` |
| `vault.go`, `vault_check.go`, `vault_rekey.go` | `docs/docs/reference/cli/vaults.md` |
| `runner.go`, `runner_register.go`, `runner_setup.go`, `runner_start.go`, `runner_unregister.go` | `docs/docs/reference/cli/runners.md` |
| `migrate.go` | `docs/docs/reference/cli/migrations.md` |
...
- Internal links are absolute from the route base `/`: **`/reference/cli/<page>`** (NOT `/cli/<page>` — that
...
- Every command-group page must be registered in **`docs/sidebars.js`** under the `label: 'CLI'` category's
  `items` array, as `'reference/cli/<page>'`. Adding a page without registering it hides it from navigation.
